...
public:
	...
	void Output( std::string file ) {
        ...
        Mat picin(bit_Info.biHeight, bit_Info.biWidth, CV_8UC3);
		for ( int i = 0 ; i < bit_Info.biHeight ; i++ ) {
			for ( int j = 0 ; j < bit_Info.biWidth ; j++ ) {

				picin.at<Vec3b>(i,j)[2]= ima[i][j].blue;
				picin.at<Vec3b>(i,j)[1]= ima[i][j].green;
				picin.at<Vec3b>(i,j)[0]= ima[i][j].red;
			}
		}
        imshow("piccv.bmp",picin);
        waitKey();
        imwrite("piccv.bmp",picin);
	}
...
public:
    ...
    CPoint operator+(const CPoint point){CPoint sum(this->x + point.x, this->y += point.y);return sum;}
    CPoint operator*(const double num){CPoint sum(this->x * num, this->y * num);return sum;}
...
class Bezier : public NoLightObj {
	VDouble3 N;//N位置
    CPoint* point;//四个控制点
    CPoint* kpoint;
    int tk1;
...
public:   
	...
	void CreateScene( ) {//从文件中读入场景的数据
        ...
        //bezier
		new_primitive = new Bezier(VDouble3(0, -4, 1), 1.0);
...
void MultiThreadSampling(int fg,int H1,void*pp ) {
	...
	for (bool end = false; !end; ) {
		...
		printf("A   guangzi   1111111111111111111\n"   );
	}
	printf("A   guangzi   222222222222222222\n"   );	
	delete[] completeRow;
}
int main() {	
	start_time = clock();	
	...
	printf("bmp3  out w=%d,h=%d\n",W,  H );
	for (int i = 0; i < H; i++) delete[] hash_d[i];
	delete[] hash_d;
		printf("bmp4  out w=%d,h=%d\n",W,  H );
	Bitmap* bmp = new Bitmap;
	camera->Output(bmp);
		printf("bmp5  out w=%d,h=%d\n",W,  H );
	bmp->Output("pic.bmp");
	delete bmp;
	delete[] photons;
	delete  scene;
