...
//本文件含有4个类，是最基础的类：颜色类ColorRGB，矢量类 VDouble3，位图类Bitmap ，材质类 MaterialQua
...
public:
	...
	void Input( std::string file ) {
		...
		RGBQUAD Pla;
		for ( int i = 0 ; i < ( int ) bit_Info.biClrUsed ; i++ ) {
			fread( ( char * ) & ( Pla.rgbBlue ) , 1 , 1 , fpi );
			fread( ( char * ) & ( Pla.rgbGreen ) , 1 , 1 , fpi );
			fread( ( char * ) & ( Pla.rgbRed ) , 1 , 1 , fpi );
		}	
	...
	void Output( std::string file ) {
		...
		for ( int i = 0 ; i < bit_Info.biHeight ; i++ ) {
			...
			byte buffer = 0;
			for (int j = 0; j < number; j++) fwrite( &buffer , 1 , 1 , fpw );
		}	
		fclose( fpw );
	}
...
public:
	...
	ColliderRecord CollideOne(Ray ray) {
		ColliderRecord colli_R;	
	  	VDouble3 B,C;	  	
	  	B = pos[2] - pos[0], C = pos[1] - pos[0];
		double dot11 = C.Dot( C),	dot01 = C.Dot( B),	  	dot00 = B.Dot( B);
	 	double inverDeno = dot01 * dot01 -  dot11 * dot00; 
		...
		if (bestCoord != -1) {
			...
			if (cost < thisCost) {
				...
				//	DivideNode( node->leftNode);
					//std::thread subThread(&DivideNode, this, node->leftNode);
					//subThread.detach();
				//} else
					DivideNode(node->leftNode);
...
public:   
	...
	void CreateScene( ) {//从文件中读入场景的数据
		...
		camera=new Camera(VDouble3(0, -8 ,2),//视点坐标 
				VDouble3(0, 1, 0),//法向量 
				16,12,//照片的象素长（或宽）---  bmp图像的象素高 
				1.0,0.75,//镜头的长（或宽）与感光点到镜头距离的比值		
				128.0,0.15,8,//景深 //dofSample=128;	aperture=0.15;	focalLen=8
		...
		new_primitive = new Sphere(VDouble3(1, 2,2.2),//球心 
				VDouble3(0, 0, 0),//矩形的x半轴
				VDouble3(0, 0, 0),0.2,//矩形的y半轴 //原点到平面的距离 
				ColorRGB(0,1,1),ColorRGB(0,1,0),//表面颜色， 吸收色
				0.0, 1.0, 0.0, 0.0,//反射系数，折射系数，漫反射系数，镜面反射 
				1.7,0.15,""  //两种介质折射率比值,偏差半径,纹理bmp文件名 
				);
		AddPrimitive( new_primitive );//一个透明r=0.2的玻璃球
		new_primitive->PreTreatment();
